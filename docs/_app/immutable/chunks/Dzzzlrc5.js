...
`).trim(),d=[])}for(const a of i.split(/\r?\n/)){if(a.startsWith("## ")){p(),r=A(a.slice(3));continue}r&&d.push(a)}return p(),{...e,sections:t}}function k(o){return o.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function O(o=""){return k(o).replace(/\*([^*]+)\*/g,"<em>$1</em>")}function l(o){return Object.fromEntries(Object.entries(o).map(([i,e])=>e==="si"?[i,!0]:e==="no"?[i,!1]:[i,e]))}const w=n(g).map(l);n(b).map(l);const x=n(v).map(l),N=n(C).map(l),B=n(f).map(l),U=n(y).map(l),G=n(_).map(l);n(z).map(l);const M=n(h),F=n(E),V=n(R),L=n(I);n(S);const H={proyecto:m(D),infraestructura:m(q),actividades:m(P),participantes:m(j)};export{N as a,H as b,U as c,L as d,F as e,O as f,w as h,B as i,M as o,V as p,x as r,G as w};
